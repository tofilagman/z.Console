...
namespace z.Console
{
    public class ConsoleApp : IConsoleApp, IDisposable
    {
        ...
        public ConsoleApp(string fileName, string cmdLine, string verb = "")
        {
            FileName = fileName;
            CmdLine = cmdLine;
            Verb = verb;
        ...
        public bool CreateNoWindow { private get; set; }
        ...
        public void WriteLine(string Data, params object[] args)
        {
        ...
        private void StartProcessAsync()
        {
            ...
            try
            {
                ...
                ProcessID = _process.Id; //get the bound process ID
            }
            catch (Exception ex)
            {
        ...
        private System.Security.SecureString GetSecureString(string str)
        {
            ...
            foreach (char c in str) ssr.AppendChar(c);
            ssr.MakeReadOnly();
            return ssr;
        }
        ...
        {
            _consoleOutputQueue.Enqueue(new ConsoleOutputEventArgs(e.Data, false, this.Tag));
            _processEvent.Set();
        }

        private void OnErrorLineReceived(object sender, DataReceivedEventArgs e)
        {
            _consoleOutputQueue.Enqueue(new ConsoleOutputEventArgs(e.Data, true, this.Tag));
            _processEvent.Set();
        }
